...
# all the files.
FIELD_ACQ_DATE = 'acq_date'
FIELD_SYS_DATE = 'sys_date'
FIELD_OS = 'os'
FIELD_FILE_FORMAT = 'file_format'
FIELD_MEDIA_TYPE = 'media_type'
FIELD_IS_PHYSICAL = 'is_physical'
FIELD_BPS = 'bps'
FIELD_MEDIA_SIZE = 'media_size'
FIELD_MD5 ='md5'

## Grouping tags
TAG_EWINFO = 'ewfinfo'
TAG_ACQ_INFO = 'acquiry_information'
TAG_EWF_INFO = 'ewf_information'
TAG_MEDIA_INF = 'media_information'

EWF_PARENT_TAGS = [TAG_EWINFO, TAG_ACQ_INFO, TAG_EWF_INFO, TAG_MEDIA_INF]

TAG_ACQ_DATE = 'acquisition_date'
TAG_ACQ_SYS = 'acquisition_system'
TAG_FILE_FORMAT = FIELD_FILE_FORMAT
TAG_SYS_DATE = 'system_date'
TAG_MEDIA_TYPE = FIELD_MEDIA_TYPE
TAG_IS_PHYSICAL = FIELD_IS_PHYSICAL
TAG_BPS = 'bytes_per_sector'
TAG_MEDIA_SIZE = FIELD_MEDIA_SIZE
TAG_HASH_DIGEST = 'hashdigest'

EWF_TAG_DICT = {
    TAG_ACQ_DATE : FIELD_ACQ_DATE,
    TAG_ACQ_SYS : FIELD_OS,
    TAG_FILE_FORMAT : FIELD_FILE_FORMAT,
    TAG_SYS_DATE : FIELD_SYS_DATE,
    TAG_MEDIA_TYPE : FIELD_MEDIA_TYPE,
    TAG_IS_PHYSICAL : FIELD_IS_PHYSICAL,
    TAG_BPS : FIELD_BPS,
    TAG_MEDIA_SIZE : FIELD_MEDIA_SIZE,
    TAG_HASH_DIGEST : FIELD_MD5
}
...
EXCEP_PARSING = 'Exception when parsing %s: %s'
...
def bcawGetXmlInfo(xmlfile):
    ...
    if xmlfile == None:
        ...
        # dummy info.
        dbrec = dict()
        dbrec[FIELD_ACQ_DATE] = NA
        dbrec[FIELD_SYS_DATE] = NA
        dbrec[FIELD_OS] = NA
        dbrec[FIELD_FILE_FORMAT] = RAW
        dbrec[FIELD_MEDIA_TYPE] = NA
        dbrec[FIELD_IS_PHYSICAL] = NA
        dbrec[FIELD_BPS] = 0
        dbrec[FIELD_MEDIA_SIZE] = NA
        dbrec[FIELD_MD5] = NA
        return dbrec
    ...
    try:
        ...
        logging.error(EXCEP_PARSING, xmlfile, e)

    root = tree.getroot() # root node
    return mapped_dict_from_element(root, EWF_PARENT_TAGS, EWF_TAG_DICT)
...
def bcawGetDfxmlInfo(dfxmlfile, img):
    ...
    try:
        ...
        logging.error('Exception caught parsing %s, error code: %s', dfxmlfile, e)
...
class BcawImages(db_login.Model):
    ...
    image_name = db_login.Column(db_login.String(60), unique=True)
...
def bcawDbSessionAdd(dbrec):
    if dbu_does_table_exist_for_img(dbrec['image_name'], 'bcaw_images'):
        logging.debug('Image %s already in bcaw_images table', dbrec['image_name'])
        return
