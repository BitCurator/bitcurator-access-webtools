...
#
# This file contains the main bca-webtools application.
#
...
from bca-webtools_utils import bca-webtools

from bca-webtools import app
import bca-webtools_db
...
def bcawBrowseImages():
    ...
    session = bca-webtools_db.bcawdb()
    ...
    for img in os.listdir(image_dir):
        if img.endswith(".E01") or img.endswith(".AFF"):
            ...
            idb = bca-webtools_db.BCAWImages.query.filter_by(image_name=img).first()
            image_db.append(idb)
...
def image(image_name):
    ...
    num_partitions = bca-webtools.num_partitions_ofimg[str(image_name)]
    part_desc = [num_partitions]

    image_index =  bcawGetImageIndex(image_name, is_path=False)
    for i in range(0, num_partitions):
        part_desc[i] = bca-webtools.partDictList[image_index][i]['desc']
